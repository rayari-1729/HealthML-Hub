...
def load_training_data(data_path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    X[:, 0] = np.random.uniform(50, 200, n_samples)  # mean_intensity
    X[:, 1] = np.random.uniform(10, 50, n_samples)  # std_intensity
    X[:, 7] = np.random.uniform(100, 10000, n_samples)  # area
    
    # Generate labels (tumor if area > threshold and intensity patterns)
    y = ((X[:, 7] > 2000) & (X[:, 0] > 100)).astype(int)
...
def train_brain_tumor_model(
    ...
    registry = {
        ...
        "known_limitations": [
            ...
            "Uses extracted features, not raw images",
            "In production, use CNN for direct image classification",
        ],
    }
