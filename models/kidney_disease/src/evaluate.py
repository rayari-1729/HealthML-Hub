"""Evaluation script for kidney disease model."""
...
def evaluate_kidney_disease_model(
    model_path: Path,
    ...
    model = KidneyDiseaseModel(model_path=model_path)
    ...
    np.random.seed(123)
    n_samples = 200
    X_test = np.random.rand(n_samples, 24)
    y_test = (X_test[:, 11] > 1.2).astype(int)
