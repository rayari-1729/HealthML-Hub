"""Training script for lung disease model."""
...
def load_training_data(data_path: Path) -> tuple[np.ndarray, np.ndarray]:
    np.random.seed(42)
    n_samples = 1000
    X = np.random.randint(0, 2, (n_samples, 23))
    y = (X[:, 10] + X[:, 14] + X[:, 15] > 1).astype(int)  # smoking + fatigue + weight_loss
    logger.info(f"Loaded {len(X)} training samples")
    return X, y


def train_lung_disease_model(
